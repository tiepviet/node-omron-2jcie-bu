...
const Controller = (options) => {
     ...
     * @return {Promise<ResponsePayloadArray>}
     */
    privateScope.execute = (commandPayload) => {
        const task = privateScope.queue.enqueue(commandPayload);
        return task.promise;
    };
    ...
    publicScope.memoryDataLong = {
        ...
        read: (data) => {
            return privateScope.execute({
                ...
                data,
            }).then((responsePayloads) => responsePayloads.map((responsePayload) => responsePayload.data));
        },
    };
    ...
    publicScope.latestDataLong = {
        ...
        read: (data) => {
            return privateScope.execute({
                ...
                data,
            }).then((responsePayloads) => responsePayloads[0].data);
        },
    };
